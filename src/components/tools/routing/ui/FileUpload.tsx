...
const FileUpload: FC<IProps> = ({ dataType }) => {
  ...
  const setLocations = useStops((state) => state.setLocations);
  const setDrivers = useDrivers((state) => state.setDrivers);
    ...
    try {
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          if (dataType === "stop") setLocations(parsedData);
          else {
  ...
  return (
    <div
      className={classNames(
        ...
        " flex aspect-square  flex-col items-center justify-center  border-4 border-dashed border-slate-300 bg-slate-100 py-5 text-center "
      )}
