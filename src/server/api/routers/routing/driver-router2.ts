...
import {
  driverSchema2,
  driverVehicleSchema2,
  vehicleSchema2,
  type DriverVehicleBundle2,
...
export const driverRouter2 = createTRPCRouter({
  setDepotVehicles: protectedProcedure
    .input(
      z.object({ data: z.array(driverVehicleSchema2), depotId: z.string() })
    )
    .mutation(async ({ ctx, input }) => {
      ...
      const res = await Promise.all(
        input.data.map(async (driverVehicle) => {
          ...
          return { driver, vehicle } as DriverVehicleBundle2;
        })
      )
        .then((data) => data)
        .catch((e) => {
  ...
  createVehicleBundles: protectedProcedure
    .input(
      z.object({
        data: z.array(driverVehicleSchema2),
        depotId: z.string(),
...
//     }),
  ...
  updateDriverDetails: protectedProcedure
    .input(z.object({ driverId: z.string(), driver: driverSchema2 }))
    .mutation(({ ctx, input }) => {
  ...
  updateVehicleDetails: protectedProcedure
    .input(z.object({ vehicle: vehicleSchema2, routeId: z.string() }))
    .mutation(async ({ ctx, input }) => {
      await ctx.prisma.vehicle.update({
        where: {
          id: input.vehicle.id,
  ...
  getDepotDrivers: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return bundles as DriverVehicleBundle2[];
    }),
